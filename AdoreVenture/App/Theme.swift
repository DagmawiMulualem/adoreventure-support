...
struct AVIdea: Identifiable, Hashable, Codable {
    // Deterministic ID based on content - same venue = same ID (prevents duplicates)
    let id: String
    ...
    // Custom initializer that creates deterministic ID from content
    init(
        title: String,
        blurb: String,
        rating: Double,
        ...
        hours: [String]? = nil,
        imageURL: String? = nil,
        imageCreditName: String? = nil,
        imageCreditLink: String? = nil,
        accessCount: Int? = nil,
        lastAccessed: Date? = nil
    ) {
        // Create deterministic ID from title only (normalized)
        // This ensures "Gardens by the Bay" always has the same ID regardless of place value
        let normalized = title.lowercased()
            .trimmingCharacters(in: .whitespacesAndNewlines)
            .replacingOccurrences(of: " ", with: "_")
        // Use URL-safe base64 and strip '/' so Firestore document IDs never contain path separators
        let base64 = normalized.data(using: .utf8)?.base64EncodedString() ?? UUID().uuidString
        self.id = base64
            .replacingOccurrences(of: "/", with: "_")
        
        self.title = title
        self.blurb = blurb
        self.rating = rating.avSanitizedStarRating
        self.place = place
        ...
        self.imageCreditName = imageCreditName
        self.imageCreditLink = imageCreditLink
        self.accessCount = accessCount
        self.lastAccessed = lastAccessed
    }

    /// JSONDecoder uses synthesized decoding and skips `init`, so bad `rating` values can slip in; rebuild through `init` to clamp.
    func rehydratedThroughInit() -> AVIdea {
        AVIdea(
            title: title,
            blurb: blurb,
            rating: rating,
...
extension AVIdea {
    static func from(dictionary: [String: Any]) -> AVIdea? {
        ...
        // Handle rating as Double or String
        let rawRating: Double
        if let ratingDouble = dictionary["rating"] as? Double {
            rawRating = ratingDouble
        } else if let ratingString = dictionary["rating"] as? String, let parsed = Double(ratingString) {
            rawRating = parsed
        } else {
            rawRating = 4.5
        }
        let rating = rawRating.avSanitizedStarRating
