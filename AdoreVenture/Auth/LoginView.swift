                            ...
                            VStack(spacing: 12) {
                                ...
                                Button {
                                    Task {
                                        ...
                                        if !success && firebaseManager.errorMessage != nil {
                                            // Only show alert if there's an actual error (not user cancellation)
                                            alertMessage = firebaseManager.errorMessage!
                                            showAlert = true
                                        }
                                    }
                                } label: {
                        ...
                        VStack(spacing: 20) {
                            ...
                            Button {
                                ...
                                if displayName.isEmpty || email.isEmpty || password.isEmpty || confirmPassword.isEmpty {
                                    ...
                                    alertMessage = "Display name must be at least 2 characters"
                                    showAlert = true
                                } else {
                                    Task {
                                        print("🔐 SignUp: Starting Firebase sign up...")
                                        let success = await firebaseManager.signUp(email: email, displayName: displayName, password: password)
                                        await MainActor.run {
                                            if success {
